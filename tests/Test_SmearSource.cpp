...
std::string split_pos(std::string pos_4d){
    std::string temp = "";
    int count = 0;
    char del = ' ';
...
// rhq insertion I/II
std::string insert_rhqI(Application &application, std::string quark, std::string gamma) {
...
// rhq insertion III
std::string insert_rhqIII(Application &application, std::string quark, std::string gamma) {
...
// rhq insertion IV
std::string insert_rhqIV(Application &application, std::string quark, std::string gamma) {
...
// rhq insertion V
std::string insert_rhqV(Application &application, std::string quark, std::string gamma) {
...
// rhq insertion VI
std::string insert_rhqVI(Application &application, std::string quark, std::string gamma) {
...
// gaussian smeared source
std::string make_SmrSrc(Application &application, std::string quark, TestInputs::SmearPar &smearPar, std::string pt_src){
    std::string sm_src = "sm_" + quark;
    
    MSource::JacobiSmear::Par smear_src;
    smear_src.gauge = "gauge";
    smear_src.width = smearPar.width;
    smear_src.iterations = smearPar.iterations;
    smear_src.orthog = smearPar.orthog;
    smear_src.source = pt_src;
    application.createModule<MSource::JacobiSmear>(sm_src,smear_src);
...
int main(int argc, char *argv[])
{
    ...
    ptPar.position = testPar.configPar.source_loc; // "0 0 0 0";
    ...
    std::string src = make_SmrSrc(application, testPar.rhqPar.quark, testPar.smearPar, "pt_src"); 
    std::string b_solver = make_rhqsolver(application, testPar.rhqPar);
    // smr src b quark for pt snk
    std::string name_q2 = make_rhqpropagator(application, testPar.rhqPar, src, b_solver, "_sm");
    // pt src b quark for pt sink
    std::string name_q2_pt = make_rhqpropagator(application, testPar.rhqPar, "pt_src", b_solver, "_pt");
    // smr src b quark with smr snk (convolution)
    MSource::Gauss::Par smrPar;
    smrPar.position = split_pos(testPar.configPar.source_loc);
    smrPar.tA = 0;  // add these to xml input so easier for any ensemble?
    smrPar.tB = 64;
    smrPar.mom = "0 0 0 0"; // always zero for b quark still, or treat this like snk mom?
    smrPar.width = 0.0; //testPar.smearPar.width;
    application.createModule<MSource::Gauss>("gauss_filter",smrPar);

    std::string gaussj = make_SmrSrc(application, "gauss_filter", testPar.smearPar, "gauss_filter");

    std::string name_q2_sm = name_q2 + "_sm";

    MSink::SourceSink::Par sPar;
    sPar.source = gaussj;
    application.createModule<MSink::SourceSink>("gaussj_snk",sPar);

    MSink::Smear::Par smPar;
    smPar.q = name_q2;
    smPar.sink = "gaussj_snk";
    application.createModule<MSink::Smear>(name_q2_sm,smPar);

//    std::string name_q2_sm = name_q2 + "_sm";
//    MSource::Convolution::Par convPar;
//    convPar.field = name_q2;
//    convPar.filter = gaussj;
//    convPar.mom = "0 0 0 0"; // same as above..?
//    application.createModule<MSource::Convolution>(name_q2_sm,convPar);
    ...
    // RHQ Improvement 2
    std::string O2_V = insert_rhqI(application, name_q2, "Identity");
    std::string O2_A = insert_rhqI(application, name_q2, "Gamma5");
    
    // RHQ Improvement 4
    std::string O4_V = insert_rhqIV(application, name_q2, "Identity");
    std::string O4_A = insert_rhqIV(application, name_q2, "Gamma5");
    ...
    for (int i = 0; i < momenta.size(); i++) {
        // dwf source
        std::string snk_mom = momenta[i];

        std::string mes = testPar.rhqPar.quark + testPar.dwfPar.quark;
        std::string fileSt = mes + "_" + squareMom(snk_mom) + "/";

        MSink::Point::Par sinkPar;
        sinkPar.mom = snk_mom;
        std::string snk_str = "pt_sink_p" + removeSpaces(snk_mom);
        application.createModule<MSink::ScalarPoint>(snk_str, sinkPar);
        application.createModule<MSink::Point>(snk_str+"_fn", sinkPar);
        ...
        std::string name_q1 = make_dwfpropagator(application, testPar.dwfPar, "pt_src", removeSpaces(snk_mom), i);
        std::string name_q1_sm = name_q1 + "_sm";
        MSink::Smear::Par smPar;
        smPar.q = name_q1;
        smPar.sink = snk_str+"_fn";
        application.createModule<MSink::Smear>(name_q1_sm,smPar);
        ...
        // RHQ Improvement 1
        std::string O1_V = insert_rhqI(application, name_q1, "Identity");
        std::string O1_A = insert_rhqI(application, name_q1, "Gamma5");
    
        // RHQ Improvement 3
        std::string O3_V = insert_rhqIII(application, name_q1, "Identity");
        std::string O3_A = insert_rhqIII(application, name_q1, "Gamma5");
        ...
        // O(0) operators
        make_contraction(application, name_q2_pt + name_q1 + "_PT_PT", name_q1, name_q2_pt, gamma_pairs, snk_str, fileSt);
        make_contraction(application, name_q2 + name_q1 + "_SM_PT", name_q1, name_q2, gamma_pairs, snk_str, fileSt);
        make_contraction(application, name_q2 + name_q1 + "_SM_SM", name_q1_sm, name_q2_sm, gamma_pairs, snk_str, fileSt);

        // O(a) operators Axial
        if (gamma_ax != "") {
            make_contraction(application, O1_Ames + "_SM_PT", O1_A, name_q2, gamma_ax, snk_str, fileSt);
            make_contraction(application, O2_Ames + "_SM_PT", name_q1, O2_A, gamma_ax, snk_str, fileSt);
            make_contraction(application, O3_Ames + "_SM_PT", O3_A, name_q2, gamma_ax, snk_str, fileSt);
            make_contraction(application, O4_Ames + "_SM_PT", name_q1, O4_A, gamma_ax, snk_str, fileSt);
        }

        // O(a) operators Vector
        if (gamma_vt != "") {
            make_contraction(application, O1_Vmes + "_SM_PT", O1_V, name_q2, gamma_vt, snk_str, fileSt);
            make_contraction(application, O2_Vmes + "_SM_PT", name_q1, O2_V, gamma_vt, snk_str, fileSt);
            make_contraction(application, O3_Vmes + "_SM_PT", O3_V, name_q2, gamma_vt, snk_str, fileSt);
            make_contraction(application, O4_Vmes + "_SM_PT", name_q1, O4_V, gamma_vt, snk_str, fileSt);
        }
    }
