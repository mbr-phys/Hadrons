...
std::string make_Z2src(Application &application, TestInputs::DwfPar &dwfPar){
    // start with standard Z2 source
    MSource::Z2::Par z2Par;
    z2Par.tA = 0; // can add this input to xml as well maybe
    z2Par.tB = 0;
    std::string z2_mom = "z2_" + dwfPar.quark + "_p000_t0";
    ...
    if (dwfPar.mom != "0 0 0") {
        ...
        z2_mom = "z2_" + dwfPar.quark + "_p" + removeSpaces(mom_str) + "_t0";
...
int main(int argc, char *argv[])
{
    ...
    // sources
    std::string src1 = make_Z2src(application, testPar.dwfPar1);

    std::string src2 = make_Z2src(application, testPar.dwfPar2);
